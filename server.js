...
// ── env helper: 양끝 따옴표/공백 제거
const clean = (v) => (v ?? '').trim().replace(/^['"]|['"]$/g, '');

// ── 환경변수
const PORT = Number(clean(process.env.PORT)) || 4000;   // Render가 PORT를 주입함
const CORS_ORIGINS = clean(process.env.CORS_ORIGINS)
  .split(',').map(s => s.trim()).filter(Boolean);

const MONGODB_URI_ADMIN = clean(process.env.MONGODB_URI_ADMIN);
const MONGODB_URI_USER  = clean(process.env.MONGODB_URI_USER);
const UPLOAD_DIR        = clean(process.env.UPLOAD_DIR) || path.join(process.cwd(), 'uploads');
...
// ── 모델 주입
// ⚠️ 리눅스(Render)는 파일명 대소문자 구분. 실제 파일명이 Product.js / user.js 인지 확인!
...
// ── 관리자 로그인 (데모: 평문 확인 / 추후 bcrypt 권장)
app.post('/admin/login', async (req, res) => {
  const { id, password } = req.body;
  try {
    const collection = adminDB.collection('admin_user');
    const admin = await collection.findOne({ id, password }); // TODO: bcrypt
